...
def rotation_y(q):
    ...
    rmatrix = np.asarray([[math.cos(q), 0.0, math.sin(q)], [0.0, 1.0, 0.0], [math.sin(q)*-1, 0.0, math.cos(q)]])
    return rmatrix


def rotation_z(q):
    '''Returns matrix representing a rotation about the z axis'''
    rmatrix = np.asarray([[math.cos(q), math.sin(q) * -1,0.0], [math.sin(q), math.cos(q), 0.0], [0.0, 0.0, 1.0]])
    return rmatrix

def rotation_x(q):
    '''Returns matrix representing a rotation about the z axis'''
    rmatrix = np.asarray([[1.0, 0.0, 0.0], [0.0, math.cos(q), math.sin(q) * -1], [0.0, math.sin(q), math.cos(q)]])
    return rmatrix
